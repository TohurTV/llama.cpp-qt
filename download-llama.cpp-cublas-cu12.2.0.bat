...
set "download_url=https://github.com/%REPO%/releases/download/%TAG%/llama-%TAG%-bin-win-cublas-cu12.2.0-x64.zip"

:: Create a temporary PowerShell script
echo Invoke-WebRequest -Uri "!download_url!" -OutFile llama-%TAG%-bin-win-cublas-cu12.2.0-x64.zip > download.ps1

:: Download the zip file using PowerShell
powershell.exe -ExecutionPolicy Bypass -File download.ps1

:: Check if the downloaded file is valid
if not exist llama-%TAG%-bin-win-cublas-cu12.2.0-x64.zip (
...
powershell -command "Expand-Archive -Path 'llama-%TAG%-bin-win-cublas-cu12.2.0-x64.zip' -DestinationPath '%TEMP_DIR%'"
...
echo Cleaning up...
rd /s /q "%TEMP_DIR%"
del llama-%TAG%-bin-win-cublas-cu12.2.0-x64.zip
