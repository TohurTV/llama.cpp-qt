...
curl -LJO https://github.com/%REPO%/releases/%TAG%/download/llama-b1280-bin-win-openblas-x64.zip

:: Extract the downloaded release zip file using Windows built-in zip utility
echo Extracting files...
powershell -command "Expand-Archive -Path 'llama-b1280-bin-win-openblas-x64.zip' -DestinationPath '%TEMP_DIR%'"
...
echo Cleaning up...
rd /s /q "%TEMP_DIR%"
del llama-b1280-bin-win-openblas-x64.zip
